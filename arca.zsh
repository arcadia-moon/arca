...
# Stores (into prompt_arca_cmd_exec_time) the execution
# time of the last command if set threshold was exceeded.
prompt_arca_check_cmd_exec_time() {
	integer elapsed
	(( elapsed = EPOCHSECONDS - ${prompt_arca_cmd_timestamp:-$EPOCHSECONDS} ))
	typeset -g prompt_arca_cmd_exec_time=
	(( elapsed > ${ARCA_CMD_MAX_EXEC_TIME:-5} )) && {
		prompt_arca_human_time_to_var $elapsed "prompt_arca_cmd_exec_time"
	}
}

prompt_arca_set_title() {
	...
	local hostname=
	if [[ -n $prompt_arca_state[username] ]]; then
...
prompt_arca_preexec() {
	if [[ -n $prompt_arca_git_fetch_pattern ]]; then
		# Detect when Git is performing pull/fetch, including Git aliases.
		local -H MATCH MBEGIN MEND match mbegin mend
		if [[ $2 =~ (git|hub)\ (.*\ )?($prompt_arca_git_fetch_pattern)(\ .*)?$ ]]; then
			# We must flush the async jobs to cancel our git fetch in order
			# to avoid conflicts with the user issued pull / fetch.
			async_flush_jobs 'prompt_arca'
		fi
	fi

	typeset -g prompt_arca_cmd_timestamp=$EPOCHSECONDS

	# Shows the current directory and executed command in the title while a process is active.
	prompt_arca_set_title 'ignore-escape' "$PWD:t: $2"

	# Disallow Python virtualenv from updating the prompt. Set it to 12 if
	# untouched by the user to indicate that Arca modified it. Here we use
	# the magic number 12, same as in `psvar`.
	export VIRTUAL_ENV_DISABLE_PROMPT=${VIRTUAL_ENV_DISABLE_PROMPT:-12}
}

# Change the colors if their value are different from the current ones.
prompt_arca_set_colors() {
	local color_temp key value
	for key value in ${(kv)prompt_arca_colors}; do
		zstyle -t ":prompt:arca:$key" color "$value"
		case $? in
			1) # The current style is different from the one from zstyle.
				zstyle -s ":prompt:arca:$key" color color_temp
				prompt_arca_colors[$key]=$color_temp ;;
			2) # No style is defined.
				prompt_arca_colors[$key]=$prompt_arca_colors_default[$key] ;;
		esac
	done
}
...
prompt_arca_preprompt_render() {
	...
	[[ -n ${prompt_arca_git_last_dirty_check_timestamp+x} ]] && git_color=$prompt_arca_colors[git:branch:cached]
	...
	if [[ $1 == precmd ]]; then
		...
		print
	elif [[ $prompt_arca_last_prompt != $expanded_prompt ]]; then
		# Redraw the prompt.
		prompt_arca_reset_prompt
	fi

	typeset -g prompt_arca_last_prompt=$expanded_prompt
}
...
prompt_arca_precmd() {
	...
	prompt_arca_set_title 'expand-prompt' '%~'
	...
	prompt_arca_reset_prompt_symbol

	# Print the preprompt.
	prompt_arca_preprompt_render "precmd"

	if [[ -n $ZSH_THEME ]]; then
		print "WARNING: Oh My Zsh themes are enabled (ZSH_THEME='${ZSH_THEME}'). Arca might not be working correctly."
		print "For more information, see: https://github.com/arcadia-moon/arca#oh-my-zsh"
		unset ZSH_THEME  # Only show this warning once.
	fi
}

prompt_arca_async_git_aliases() {
	setopt localoptions noshwordsplit
...
prompt_arca_async_tasks() {
	...
	local -H MATCH MBEGIN MEND
	if [[ $PWD != ${prompt_arca_vcs_info[pwd]}* ]]; then
	...
	async_job "prompt_arca" prompt_arca_async_vcs_info

	# Only perform tasks inside a Git working tree.
	[[ -n $prompt_arca_vcs_info[top] ]] || return

	prompt_arca_async_refresh
}
...
prompt_arca_async_refresh() {
	...
	integer time_since_last_dirty_check=$(( EPOCHSECONDS - ${prompt_arca_git_last_dirty_check_timestamp:-0} ))
	if (( time_since_last_dirty_check > ${ARCA_GIT_DELAY_DIRTY_CHECK:-1800} )); then
		unset prompt_arca_git_last_dirty_check_timestamp
...
prompt_arca_check_git_arrows() {
	setopt localoptions noshwordsplit
	local arrows left=${1:-0} right=${2:-0}

	(( right > 0 )) && arrows+=${ARCA_GIT_DOWN_ARROW:-⇣}
	(( left > 0 )) && arrows+=${ARCA_GIT_UP_ARROW:-⇡}

	[[ -n $arrows ]] || return
	typeset -g REPLY=$arrows
}

prompt_arca_async_callback() {
	setopt localoptions noshwordsplit
	...
	case $job in
		...
		prompt_arca_async_vcs_info)
			...
			if [[ $info[top] = $prompt_arca_vcs_info[top] ]]; then
				# If the stored pwd is part of $PWD, $PWD is shorter and likelier
				# to be top-level, so we update pwd.
				if [[ $prompt_arca_vcs_info[pwd] = ${PWD}* ]]; then
					prompt_arca_vcs_info[pwd]=$PWD
				fi
			else
		...
		prompt_arca_async_git_aliases)
			if [[ -n $output ]]; then
				...
				prompt_arca_git_fetch_pattern+="|$output"
			fi
			;;
		prompt_arca_async_git_dirty)
			local prev_dirty=$prompt_arca_git_dirty
			if (( code == 0 )); then
				unset prompt_arca_git_dirty
			else
				typeset -g prompt_arca_git_dirty="*"
			fi

			[[ $prev_dirty != $prompt_arca_git_dirty ]] && do_render=1

			# When `prompt_arca_git_last_dirty_check_timestamp` is set, the Git info is displayed
			...
			(( $exec_time > 5 )) && prompt_arca_git_last_dirty_check_timestamp=$EPOCHSECONDS
			;;
		prompt_arca_async_git_fetch|prompt_arca_async_git_arrows)
			# `prompt_arca_async_git_fetch` executes `prompt_arca_async_git_arrows`
			# after a successful fetch.
			case $code in
				0)
					local REPLY
					prompt_arca_check_git_arrows ${(ps:\t:)output}
					if [[ $prompt_arca_git_arrows != $REPLY ]]; then
						typeset -g prompt_arca_git_arrows=$REPLY
						do_render=1
					fi
					;;
				99|98)
					...
					;;
				*)
					# Non-zero exit status from `prompt_arca_async_git_arrows`,
	...
	if (( next_pending )); then
		(( do_render )) && typeset -g prompt_arca_async_render_requested=1
		return
	fi

	[[ ${prompt_arca_async_render_requested:-$do_render} = 1 ]] && prompt_arca_preprompt_render
...
prompt_arca_reset_prompt_symbol() {
	prompt_arca_state[prompt]=${ARCA_PROMPT_SYMBOL:-❯}
}

prompt_arca_update_vim_prompt_widget() {
	setopt localoptions noshwordsplit
	prompt_arca_state[prompt]=${${KEYMAP/vicmd/${ARCA_PROMPT_VICMD_SYMBOL:-❮}}/(main|viins)/${ARCA_PROMPT_SYMBOL:-❯}}

	prompt_arca_reset_prompt
}

prompt_arca_reset_vim_prompt_widget() {
	setopt localoptions noshwordsplit
	prompt_arca_reset_prompt_symbol
...
prompt_arca_state_setup() {
	setopt localoptions noshwordsplit

	# Check SSH_CONNECTION and the current state.
	local ssh_connection=${SSH_CONNECTION:-$PROMPT_ARCA_SSH_CONNECTION}
	...
	if [[ -z $ssh_connection ]] && (( $+commands[who] )); then
		...
		if [[ $who_out =~ "\(?($reIPv4|$reIPv6|$reHostname)\)?\$" ]]; then
			...
			export PROMPT_ARCA_SSH_CONNECTION=$ssh_connection
		fi
		unset MATCH MBEGIN MEND
	fi
...
prompt_arca_setup() {
	...
	# Borrowed from `promptinit`. Sets the prompt options in case Arca was not
	...
	# Set the colors.
	typeset -gA prompt_arca_colors_default prompt_arca_colors
	...
	add-zsh-hook precmd prompt_arca_precmd
	add-zsh-hook preexec prompt_arca_preexec

	prompt_arca_state_setup

	zle -N prompt_arca_reset_prompt
	zle -N prompt_arca_update_vim_prompt_widget
	zle -N prompt_arca_reset_vim_prompt_widget
	if (( $+functions[add-zle-hook-widget] )); then
		add-zle-hook-widget zle-line-finish prompt_arca_reset_vim_prompt_widget
		add-zle-hook-widget zle-keymap-select prompt_arca_update_vim_prompt_widget
	fi
	...
	typeset -ga prompt_arca_debug_depth
	prompt_arca_debug_depth=('%e' '%N' '%x')
	...
	local -A ps4_parts
	ps4_parts=(
		depth 	  '%F{yellow}${(l:${(%)prompt_arca_debug_depth[1]}::+:)}%f'
		compare   '${${(%)prompt_arca_debug_depth[2]}:#${(%)prompt_arca_debug_depth[3]}}'
		main      '%F{blue}${${(%)prompt_arca_debug_depth[3]}:t}%f%F{242}:%I%f %F{242}@%f%F{blue}%N%f%F{242}:%i%f'
